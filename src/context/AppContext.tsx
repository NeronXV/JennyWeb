...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [grado, setGrado] = useState('5.º');
  const [grupo, setGrupo] = useState('A');
  const [trimestre, setTrimestre] = useState('1.º');
  ...
  // Campos Formativos
  const [camposFormativos] = useState<CampoFormativo[]>([
  ...
  // Recalculates single student average
  ...
  // Keep student averages updated when grades or configs change
  useEffect(() => {
  ...
  const addAlumno = (newAl: Omit<Alumno, 'id' | 'promedio' | 'asistenciasCount' | 'faltasCount' | 'retardosCount' | 'calificaciones'>) => {
    const id = 'a' + (alumnos.length + 1);
    
    // Default mock grades structure
  ...
  const saveAsistencia = (fecha: string, status: { [alumnoId: string]: AsistenciaStatus }) => {
    ...
    // Update individual attendance counts based on history
    setAlumnos(prevAlumnos => 
      prevAlumnos.map(al => {
        let pres = 50; // base mock assistance count
        let fal = 0;
        let ret = 0;
  ...
  const updateCalificacion = (
    ...
    setAlumnos(prevAlumnos => 
      prevAlumnos.map(al => {
        ...
        const campoCal = { ...updatedCalificaciones[campoId] };
  ...
  // Add a new activity or task column (initializes with 8 for all students)
  ...
  const addProducto = (newProd: Omit<Producto, 'id'>) => {
    ...
    // Recalculate lote remaining products and totals
    setLotes(prevLotes => 
      prevLotes.map(l => {
  ...
  const registrarVenta = (newVenta: Omit<Venta, 'id' | 'fecha'>) => {
    ...
    // Update product state to 'Vendido'
